...
- `internal/client/` — the one HTTP wrapper (`client.go`, timeout + safe 429/5xx backoff + refresh-on-401) + `TokenSource` (`auth.go`) + wire contract (`types.go`, field names match the server exactly) + `APIError`.
